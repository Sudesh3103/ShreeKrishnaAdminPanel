...
const ProductOffers = () => {
  ...
  const handleAddOffer = () => {
    ...
    setShowModal(false);
    setFormData({ code: "", discountType: "Fixed", discountValue: "", maxDiscount: "", expiryDate: "", status: "Inactive" });
  };
    ...
    if (sortConfig.key) {
      sortable.sort((a, b) => {
        ...
        if (sortConfig.key === "code") {
          ...
          aValue = a.id;
          bValue = b.id;
        } else if (sortConfig.key === "discountType") {
          aValue = a.discountType.toLowerCase();
          bValue = b.discountType.toLowerCase();
        } else if (sortConfig.key === "discountValue") {
          aValue = parseFloat(a.discountValue) || 0;
          bValue = parseFloat(b.discountValue) || 0;
        } else if (sortConfig.key === "maxDiscount") {
          aValue = parseFloat(a.maxDiscount) || 0;
          bValue = parseFloat(b.maxDiscount) || 0;
    ...
    <div className="container py-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <small className="text-muted">Coupon List</small>
        </div>
        <button className="btn btn-warning d-flex align-items-center gap-2" onClick={() => { setShowModal(true); setEditMode(false); setFormData({ code: "", discountType: "Fixed", discountValue: "", maxDiscount: "", expiryDate: "", status: "Inactive" }); }}>
          ...
          <table className="table table-bordered">
            <thead className="table-light">
              <tr>
                ...
                <th style={{ cursor: "pointer" }} onClick={() => handleSort("code")}>Coupon Code {renderSortIcon("code")}</th>
                <th style={{ cursor: "pointer" }} onClick={() => handleSort("discountType")}>Discount Type {renderSortIcon("discountType")}</th>
                <th style={{ cursor: "pointer" }} onClick={() => handleSort("discountValue")}>Discount Value {renderSortIcon("discountValue")}</th>
                <th style={{ cursor: "pointer" }} onClick={() => handleSort("maxDiscount")}>Max Discount {renderSortIcon("maxDiscount")}</th>
              ...
              {currentOffers.length > 0 ? (
                currentOffers.map((offer, index) => (
                  <tr key={offer.id}>
                    ...
                    <td>₹{parseFloat(offer.discountValue || 0).toFixed(2)}</td>
                    <td>₹{parseFloat(offer.maxDiscount || 0).toFixed(2)}</td>
        ...
        <div className="d-flex justify-content-between align-items-center mt-3">
          ...
          <div>
            ...
            {[...Array(totalPages)].map((_, i) => (
              <button
                key={i}
                className={`btn btn-sm me-1 ${currentPage === i + 1 ? "btn-warning" : "btn-outline-secondary"}`}
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">{editMode ? "Edit Coupon" : "Add Coupon"}</h5>
              ...
              <div className="modal-footer">
                ...
                {editMode ? (
                  <button className="btn btn-warning text-white" onClick={handleUpdateOffer}>
                    Update
                  </button>
                ) : (
                  <button className="btn btn-warning text-white" onClick={handleAddOffer}>
                    Save
                  </button>
                )}
              </div>
